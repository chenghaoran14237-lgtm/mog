...
def add_chapter_one(doc: Document) -> None:
    ...
    for text in [
        ...
        "从国家卫生健康信息化建设要求看，医疗信息系统不仅要保存业务数据，还需要在数据标准、业务闭环、质量控制和安全管理方面形成制度化支撑[1]。电子病历应用水平评价也把结构化记录、数据共享、闭环管理和过程留痕作为衡量医院信息化水平的重要维度[2]。虽然本文系统面向个人健康资料而非医院核心业务系统，但其资料处理链路仍然需要借鉴这些要求：原始文件不能丢失，OCR 和标准化结果不能互相覆盖，指标和报告之间应建立可追踪关系。",
        "个人健康资料中包含大量敏感个人信息和健康相关信息。GB/T 35273-2020 对个人信息处理活动中的目的明确、最小必要、访问控制、安全审计和个人敏感信息保护提出了规范性要求[3]。因此，本文系统在设计时没有把所有文档直接送入不可控的外部接口，而是通过 Provider 抽象隔离外部服务，并在 provider_events、task_events 和 audit_report_events 中记录调用状态。这样的设计有助于后续部署时进一步加入脱敏、权限控制和审计日志导出。",
        "从毕业设计选题角度看，医疗资料审计辅助系统比普通健康问答系统更适合工程实现。普通问答系统的重点是回答质量，而审计辅助系统的重点是资料是否完整、指标是否异常、结论是否有来源、报告是否越界。前者容易变成大模型提示词演示，后者需要数据库建模、任务队列、状态机编排、证据绑定和前端可视化共同支撑。本文选择后者，是为了让课题能够体现计算机专业毕业设计中“需求分析、系统设计、详细实现、测试验证”的完整过程[14-15]。",
    ...
    for text in [
        ...
        "国内电子病历文本挖掘研究较早关注非结构化病历文本如何转化为可计算知识。吴宗友等将电子病历文本挖掘任务概括为数据清洗、数据集成、命名实体识别、关系抽取、信息检索和临床问答等环节[4]。杜晋华等进一步围绕中文电子病历命名实体识别总结疾病、症状、检查、药物和身体部位等实体类型，并指出中文病历文本存在术语密集、表达不规范、标注语料不足等问题[5]。这些研究说明，医疗文本处理需要先处理文本质量和结构化问题，不能直接跳到报告生成。",
        "在具体模型方法方面，韩普等提出融合外部语义特征、深度学习模型和 CRF 输出的中文电子病历实体识别方法[6]，崔少国等则从汉字图像、五笔编码、医学词典和 Lattice 结构等角度补充语义与边界信息[7]。这些工作重点提升实体识别精度，而本文重点不在训练新的 NER 模型，而是在工程系统中保留原文、版本、指标和证据关系。二者并不冲突：专门模型可以作为未来 Provider 或标准化组件接入，本文当前实现先解决数据链路和审计可追溯问题。",
        "真实世界数据治理研究对本文也有直接启发。盖彦蓉等指出，真实世界中文电子病历的数据质量、术语差异、数据稀疏和治理流程会影响知识抽取效果[8]。本文面对的个人体检报告、化验单和病历摘要同样存在模板差异、单位差异和叙事表述差异。因此，系统设计中将 OCR 原文、normalized_payload、document_versions、measurements 和 audit_report_events 分开保存，使后续即使标准化规则或模型发生变化，也能回到当时的数据版本解释报告结果。",
        ...
        "国外研究中，RAG、ReAct 和多 Agent 框架更多从通用自然语言处理和复杂任务求解角度提出方法。Lewis 等的 RAG 工作强调将参数化模型和可检索外部记忆结合，以提高知识密集型任务的事实性和可更新性[19]；Yao 等提出的 ReAct 将推理和行动交替组织，使模型能够利用外部工具或环境反馈修正任务过程[20]；Wu 等提出的 AutoGen 强调多个可配置 Agent 之间的会话协同[21]。这些方法给本文的启示是：医疗资料审计不应把模型输出当作唯一结果，而应把检索、工具调用、证据绑定和审查节点纳入流程。",
        "在医疗大模型方向，国内外研究均强调机遇与风险并存。国内文献从医疗服务、医学研究、医学教育、知识增强和基础模型体系等角度梳理了生成式大模型的应用空间，也反复指出准确性、可解释性、隐私保护、伦理和结果可控性等问题[9-13]。Thirunavukarasu 等从国际视角讨论 LLM 在医学中的潜力与风险[23]，Wornow 等则特别提醒 EHR 数据的异质性、缺失性和编码差异会削弱模型在真实医疗系统中的可靠性[24]。这也是本文把系统定位为“审计辅助”而不是“诊断系统”的主要原因。",
...
def add_chapter_two(doc: Document, snapshot: dict) -> None:
    ...
    for text in [
        ...
        "标准化工作的第一层是格式标准化，即把不同医院、体检机构或实验室给出的 PDF、图片、文本报告统一进入文件表和 OCR 表；第二层是语义标准化，即把“空腹血糖”“葡萄糖”“GLU”等可能指向同一指标的表达映射到统一字段；第三层是过程标准化，即记录每一次 OCR、标准化和报告生成的版本关系。国家医院信息化建设标准和电子病历应用水平评价虽然面向医院系统，但其中关于数据质量、业务闭环和过程留痕的要求，对个人健康资料管理系统同样具有借鉴意义[1-2]。",
        "从隐私和安全角度看，医疗资料中的姓名、年龄、检查编号、报告日期、检查结果和病史描述都可能构成敏感个人信息。个人信息安全规范强调处理个人信息应遵循目的明确、最小必要和安全保障原则[3]。因此，本文系统在本地毕业设计阶段虽然以 Mock 数据为主，但数据模型仍按真实系统思路保留 user_id 隔离、任务事件、Provider 调用事件和审计事件。这样做不是为了增加复杂度，而是为了让系统具备后续上线时扩展权限控制、脱敏和日志审计的基础。",
        ...
        "上述 NER 研究通常以标注数据集和实体边界识别为重点，适合解决疾病、症状、检查和药物等实体抽取问题[5-7]。本文的输入材料则包含体检总检、检验报告、影像结论和病历摘要，其结构不完全等同于医院电子病历。体检报告中的数值指标需要支持趋势比较，影像结论中的自然语言需要保留原文依据，病历摘要中的既往史和用药信息需要作为叙事事实参与审计。由此可见，本文更接近“工程化标准化与证据管理”问题，而不是单纯的实体识别模型问题。",
    ...
    for text in [
        ...
        "将上述研究映射到本文系统，可以发现医疗大模型的价值主要体现在三个环节。第一是 OCR 后的文本理解和标准化，模型可以辅助识别报告日期、文档类型、叙事事实和常见指标；第二是知识解释，模型可以把异常指标和医学知识来源组织成用户可读的审计说明；第三是报告生成，模型可以把多个审计节点的结构化输出整合为完整报告。但是，每个环节都必须有结构化约束、证据绑定和安全审查，否则生成式能力越强，越容易产生看似合理但无法追溯的结论[9-13]。",
        ...
        "EHR 和个人体检资料之间虽然来源不同，但都具有异质性和上下文依赖性。Wornow 等关于 EHR 基础模型的讨论提醒本文不能把单次模型输出视为可靠事实[24]。例如，血糖、肝功能、炎症指标等数值是否异常，既取决于指标值，也取决于单位、参考范围、报告日期和上下文；影像结论是否需要关注，也需要结合原文描述和报告类型。因此，本文采用 RAG 知识检索、结构化指标、原文证据和安全审查共同约束模型输出。",
        ...
        "知识增强医学语言模型研究也指出，医学知识图谱、多模态数据和外部知识库能够提升模型的专业性与可信性[10]。本文没有构建完整医学知识图谱，而是采用更适合毕业设计规模的轻量知识块方案：将医学审计知识和公开医学手册摘要整理为 knowledge_chunks，并在综合审计报告中由 knowledge_retrieval_agent 按异常指标和风险点检索。该设计比完全依赖模型参数更容易解释，也便于在论文中展示知识来源、命中片段和最终报告之间的关系。",
    ...
    for text in [
        ...
        "从方法选择上看，RAG、ReAct、AutoGen 和 LangGraph 关注的问题并不相同。RAG 解决“回答时能否利用外部知识”的问题[19]，ReAct 解决“模型能否交替推理和行动”的问题[20]，AutoGen 解决“多个 Agent 如何通过会话协作”的问题[21]，LangGraph 则更强调长运行、有状态、可持久化和可观测的工作流[22]。本文选择 LangGraph 作为综合审计报告主流程，并不是否定其他方法，而是因为医疗审计需要保存状态、事件、证据和回退路径。",
    ]:
        add_body(doc, text)
    method_rows = [
        ["RAG", "外部知识检索与生成结合", "可提供知识来源，缓解纯参数模型事实性不足", "用于 knowledge_retrieval_agent 和报告知识来源展示"],
        ["ReAct", "推理轨迹与外部行动交替", "适合需要查询、工具调用和环境反馈的任务", "用于理解审计节点需要先查询、再生成、再复核的流程思想"],
        ["AutoGen", "多 Agent 会话协作", "适合开放式复杂任务和多角色对话", "借鉴职责拆分思想，但不采用完全开放对话"],
        ["LangGraph", "状态图、条件边和持久化工作流", "适合长运行、有状态、可回环、可观察的 Agent 流程", "作为综合审计报告核心编排方式"],
    ]
    doc.add_page_break()
    add_table(doc, "表2-2 Agent 工作流相关方法对比", ["方法", "核心思想", "适用特点", "本文采用方式"], method_rows)
    for text in [
        ...
        "这种设计使“多 Agent”具有可解释的工程含义。本文中的 Agent 不是随机对话角色，而是能够在状态图中定位的处理节点；每个节点的输入来自 GraphState，输出写回 GraphState，执行过程写入 audit_report_events。前端展示的节点高亮、边流转和 visit_count 都来自后端真实事件。这样，论文中关于多 Agent 协作的描述能够对应数据库表、后端代码和页面截图，而不是停留在概念层面。",
    ...
    add_table(doc, "表2-3 Provider 抽象及当前配置", ["抽象接口", "当前配置", "功能说明"], rows)
    for text in [
